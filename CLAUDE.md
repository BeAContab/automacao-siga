...
4. **Extração SIGA** — [src/extraction/siga_extractor.py](src/extraction/siga_extractor.py) (`SigaContributorExtractor`) é o módulo maior e mais importante (~3900 linhas): abre cada contribuinte por CNPJ, navega pelas abas NF-e/NFC-e/CT-e/Malha Fiscal/Débitos Fiscais, solicita os detalhamentos dos relatórios, controla `PendingDetailRequest`s e depois varre a "Central de Downloads" do SIGA para casar e baixar os arquivos resultantes (lógica de correspondência aproximada via `DownloadLookupTarget`/`DownloadRowMatch`, já que a lista da central de downloads não mapeia 1:1 com as solicitações). Lança `TaxpayerNotFoundError` quando um CNPJ não tem cadastro ativo, para que o loop de lote pule e continue em vez de abortar. Ao decidir se abre um mês e ao pedir o detalhamento de NF-e/CT-e, sempre lê/clica a coluna "Autorizadas"/"Autorizados" da tabela "Indicadores por Mês" (`_collect_grouped_indicator_metrics`/`_click_group_cell`, que resolvem `rowspan`/`colspan` do cabeçalho de 3 linhas dessa tabela — "MÊS" ocupa a 1ª sozinha com `rowspan=3`) — nunca a primeira coluna bruta ("Emitidas"/"Emitidos"), que mistura documentos cancelados/denegados no mesmo arquivo (confirmado ao vivo em 16/09/2026: uma NF-e e um CT-e cancelados apareciam juntos com os autorizados no mesmo detalhamento). Para NF-e (`detail_mode="reports"`) o clique de abertura do mês continua no nome do mês (revela a quebra Interna/Interestadual/Externa), mas cada recorte geográfico também é lido/clicado pela coluna "Autorizadas"; para CT-e/NFC-e (`detail_mode="authorized"`) o clique de abertura já vai direto na QTD da coluna "Autorizadas/Autorizados", sem depender do nome do mês. Mantém fallback para o clique antigo (nome da linha) se a coluna não for localizada.
4b. **Extração NFC-e** — [src/extraction/nfce_extractor.py](src/extraction/nfce_extractor.py) (`NfceBatchExtractor`, `NfceSessionManager`, `NfceDownloadManager`) é um segundo modo de extração, portado de um script standalone (`importação/NFCE/codigo-fonte/xml nfce.py`, fora do pacote `src/`) para reutilizar `BrowserSession`/`selenium_compat.py` em vez de gerenciamento próprio de driver. Faz login automático no portal `servicos.sefaz.ce.gov.br` com CPF/senha (`Settings.nfce_cpf`/`nfce_senha`, preenchidos em runtime a partir de campos na própria GUI — `src/gui.py`, `nfce_cpf_var`/`nfce_senha_var` — e nunca persistidos em `.env`, planilha ou log), resolve empresas por IE/CNPJ via uma planilha-base (`load_cnpj_ie_base`, colunas CNPJ/IE obrigatórias e COD opcional), busca as chaves de 44 dígitos numa pasta configurável, num arquivo `.xlsx` único, **ou coladas manualmente** (aba "Colar chaves" da interface — `extract_nfce_keys_from_text`/`group_nfce_keys_by_cnpj`, sem arquivo de origem então sem detecção automática de direção; um seletor opcional na própria aba classifica o lote inteiro colado como Saída/Entrada/não classificado) — em qualquer um dos três casos (`find_keys_files_for_cnpj`/`collect_nfce_keys_for_cnpj`/`NfceBatchExtractor._chaves_para_cnpj`), toda chave é filtrada pelo CNPJ do emitente embutido nela mesma via `cnpj_from_access_key` (posições 7-20), garantindo que uma fonte com chaves de várias empresas misturadas seja corretamente separada. Pula chave já baixada em execução anterior (`NfceDownloadManager.ja_baixado`, busca recursiva na pasta da empresa inteira — não só na subpasta da direção atual, pois uma chave pode ter sido classificada de forma diferente numa execução passada). Recarrega a página de consulta antes de cada chave, exceto a primeira de cada passada (`_processar_empresa`) — confirmado ao vivo, direto no navegador (fora da automação), que o portal só exibe de forma confiável o botão de baixar XML na 1ª consulta feita após a página carregar: da 2ª em diante o botão simplesmente não aparece mais, mesmo com a nota já buscada com sucesso pelo próprio portal (a chamada completa, só o popup não é exibido) — recarregar evita esse travamento binário em vez de só descobri-lo tarde via o gatilho de falhas consecutivas. Se a sessão cair e nem o relogin leve nem o protocolo de recuperação total resolverem, `NfceSessionManager.recuperar_sessao` cai por fim no mesmo retry indefinido de `autenticar()` — sem essa 3ª camada, um lote muito longo (muitas empresas/horas) que esbarrasse numa instabilidade mais teimosa desistiria da empresa atual e de todas as seguintes, em cascata. Baixa as chaves com renovação proativa de sessão (a cada `nfce_session_renewal_seconds`, padrão 480s) e um protocolo de recuperação total em caso de sessão instável. A pasta de saída por empresa segue o mesmo padrão `COD - EMPRESA - CNPJ` do SIGA — padronizado na v2.5.0, antes era só `IE - EMPRESA`. O COD é resolvido em 2 níveis: 1º a planilha-base CNPJ/IE selecionada na tela; 2º, se essa não tiver COD pra empresa, a planilha-base `COD EMP CNPJ.xlsx` (mesma rede de segurança que o modo NF-e já usa) — só cai em `"SEM-COD"` se nenhuma das duas resolver. Dentro da pasta da empresa, os XMLs são separados em subpastas `Notas de Saída`/`Notas de Entrada` conforme o relatório de origem no SIGA seja do perfil "Emissor" (a empresa emitiu) ou "Destinatario" (a empresa recebeu) — detectado pelo nome do arquivo (`tipo_nota_do_nome_arquivo` em `spreadsheet.py`) em `collect_nfce_keys_for_cnpj`, que agora agrupa as chaves por direção em vez de devolver uma lista simples; sem essa detecção (nome fora do padrão do SIGA), o XML cai direto na pasta da empresa, sem subpasta extra.
4c. **Extração NF-e (Meu DANFE)** — [src/extraction/meudanfe_extractor.py](src/extraction/meudanfe_extractor.py) (`MeudanfeBatchExtractor`, `MeudanfeChromeWorker`) é o terceiro modo de extração, portado de um projeto standalone (`importação/NFE 2/automacao-meu-danfe/main.py`, fora do pacote `src/`) — substitui o script Playwright (`importação/NFE/NFE_XML.py`) que havia sido avaliado e rejeitado por incompatibilidade de motor (ver `brain/2026-08-19-nfe-playwright-nao-integrado.md` e a reversão em `brain/2026-08-20-nfe-meudanfe-integrado.md`). Diferente de SIGA/NFC-e, **não usa `BrowserSession`**: é uma consulta pública anônima (sem login) no site `meudanfe.com.br` por chave de acesso de 44 dígitos, e gerencia seus próprios drivers `undetected_chromedriver` — 1 a 4 instâncias em paralelo (`Settings.nf_meudanfe_max_workers`), cada uma com perfil e pasta de download temporária isolados, já que a resolução do captcha Cloudflare Turnstile foi validada especificamente contra as camuflagens do `undetected_chromedriver`. Lê a coluna "Chave NF-e" de qualquer planilha `.xlsx` numa pasta configurável **ou num arquivo único** (`Settings.nf_meudanfe_input_folder`, tipicamente a própria saída do modo SIGA, que já tem essa coluna; arquivos de lock do Excel `~$...` são ignorados, e chaves que não sejam modelo NF-e/55 — `is_nfe_key` — são descartadas com aviso, mesmo vindas da coluna certa) e baixa XML/PDF para `<pasta>/<COD - EMPRESA - CNPJ>/[Notas de Saída|Notas de Entrada/]{XML,PDF}/<chave>.{xml,pdf}`. A subpasta `Notas de Saída`/`Notas de Entrada` vem do perfil do relatório de origem no SIGA ("Emissor"/"Destinatario", detectado pelo nome do arquivo via `tipo_nota_do_nome_arquivo` em `spreadsheet.py`); sem essa detecção, cai direto em `{XML,PDF}`, sem subpasta extra. A pasta da empresa é resolvida **por chave**, não por arquivo (`montar_pasta_destino_chave`), priorizando o CNPJ embutido na própria chave contra a planilha-base `COD EMP CNPJ.xlsx` (instalada ao lado do `.exe`, `Settings.cod_empresa_cnpj_base_path`) — só cai para o nome da pasta de entrada (convenção `COD - EMPRESA - CNPJ` do SIGA) quando o CNPJ não consta na base, e para `SEM-COD - SEM-EMPRESA - <cnpj>` se nem isso resolver; essa prioridade (CNPJ da chave antes do nome da pasta) é o que resolve corretamente matriz/filiais mesmo quando a entrada vem de uma estrutura de pastas genérica (ex. `0001`/`0002` por filial, sem CNPJ no nome) — no caso comum (entrada = saída do próprio SIGA) o resultado é idêntico ao nome que o SIGA já deu à pasta. As chaves que falharem na primeira passada (`MeudanfeBatchExtractor.executar_lote`) ganham uma retentativa automática única após uma espera configurável (`Settings.nf_meudanfe_retry_wait_seconds`, padrão 60s) — falhas em lotes reais mostraram não ter relação com o conteúdo do documento, e sim instabilidade intermitente do site sob volume alto (v2.5.0).
4d. **Cadeia completa (SIGA → NF-e → NFC-e)** — [src/extraction/chain_extractor.py](src/extraction/chain_extractor.py) (`ChainBatchExtractor`) é um quarto modo que orquestra os três extratores acima em sequência numa única execução, sem alterar nenhum deles: roda a extração SIGA e, na sequência, usa a própria pasta de saída do SIGA como `input_folder` do NF-e (`MeudanfeBatchExtractor`) e como `keys_folder` do NFC-e (`NfceBatchExtractor`) — ambos já reconhecem nativamente a convenção de pastas `COD - EMPRESA - CNPJ` do SIGA, então não é preciso nenhuma transformação de dados entre etapas. As empresas do NFC-e são resolvidas automaticamente via `discover_selectable_companies` (sem etapa manual de revisão, ao contrário do modo NFC-e isolado). Falhas parciais na etapa SIGA não interrompem a cadeia — ela segue para NF-e/NFC-e com o que foi baixado; só um cancelamento explícito do usuário (`stop_extraction`) interrompe entre etapas. Exposto via `Api.start_chain_extraction` (`src/webui/api.py`). Cada etapa tem sua própria barra de progresso 0-100% na interface (`data-chain-only` em `index.html`, alternando com a barra única dos outros modos via `data-hide-in-chain`) — `on_siga_row_processed`/`on_nfce_row_processed` (este último novo em `run_batch_in_context`, chamado ao final de cada empresa) dão o done/total de cada etapa, e o total da etapa NF-e é obtido via `listar_planilhas_nfe` assim que ela começa; `JsBridge.set_stage_progress`/`window.sigaSetStageProgress` despacham pra barra certa.
...
O 4º modo "Cadeia Completa" (`data-mode="chain"`) reaproveita inteiramente a tela e o estado do modo SIGA na interface pywebview — mesma grade de empresas, mesma importação de planilha/entrada manual (`gridMode()` em `web/src/js/main.js` traduz o modo ativo para `'siga'` sempre que o código precisa de uma chave de estado/DOM ligada à grade) — e só acrescenta um bloco de campos extras (`[data-chain-only]` em `web/src/index.html`: CPF/senha e planilha-base CNPJ/IE do NFC-e; Chrome.exe opcional e Chromes em paralelo do NF-e), visível só nesse modo. `web/src/js/screens.js` (`runChain`) monta o payload e chama `Api.start_chain_extraction`.
